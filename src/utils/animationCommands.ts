...
export function createAnimationClickStepInSlide(
  ...
  /** startMs is Sequence-local and intentionally survives this ownership move. */
...
export function updateAnimationClickStepInSlide(
  ...
  const sequenceChanged =
    nextName !== sequence.name || !stringArraysEqual(nextClipIds, sequence.clipIds);
 ...
 * Step. The command is immutable so App.commitProjectChange can record it as one
 * Undo/Redo transaction when the Phase 6 editor invokes it.
 */
